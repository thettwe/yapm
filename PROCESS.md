...
**The E2E isolation contract** — every Playwright test starts from the state the server leaves behind at boot: one workspace and nothing else. Import `test` from `apps/web/e2e/fixtures`, never from `@playwright/test`; never create a browser context by hand; never encode fixture size or machine speed as a constant. The first two are enforced by `node scripts/check-boundaries.mjs` (rule 7), because a spec that opts out of the reset cannot report its own leak. The full rule, and how to run the suite locally, is in [`apps/web/e2e/README.md`](apps/web/e2e/README.md).
