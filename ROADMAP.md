...
| — | `zero-reconnect` | **Maintenance (P1).** Sync survives an idle tab: the query/mutate endpoints answer a presented-but-invalid credential with `401` instead of a `200` that strands the client group; the client re-mints and reconnects from every non-terminal broken state under jittered backoff (1s → 30s cap); the sync token refreshes proactively; and the connection pill becomes a tokenized, keyboard-operable, announced reconnecting state. Closes the CI gap that let it ship — reconnection is now covered end to end | ✅ built (feat/zero-reconnect) |

**V1 PM core complete** (changes 1–7): with `projects-roadmap` built, the whole tracker — workspace/auth, design system, issues, board, cycles, triage, and projects with a roadmap — is in place. With `connectors` (change 8) built, the delivery-truth wedge is live: GitHub PR/CI/deploy state feeds the issue-row reality strip and divergence flag. With `ai` (change 9) built, the BYO-key AI foundation and its first flagship — the team-internal cycle digest — are in. With `retro-board` (change 10) built, the team also has the surface where it decides what to change: a data-seeded retrospective with storage-layer anonymity and an action → issue loop, shipping with no AI in it at all. What remains are AI *features* on the change-9 foundation (the governed PM-facing digest, the **AI-facilitated** retro that drafts on top of the retro that now exists), not core project management.
