...
| **Connector framework** | Provider-neutral `ConnectorDefinition` + `WorkGraphMutation` firewall in `packages/schema` | The only shape feature code sees is the mutation union, so a second connector (GitLab) needs no reality-strip/query/row change — and none for **opt-in status automation** either, the second thing inherited from the union: the `open → In Review` / `merged → Done` decision is a pure function in `packages/schema` over the pull-request variant's state, its own source timestamp, and the rows it names, so `apps/server/src/connectors/github/` contains no reference to issue status at all (a `git diff --stat` over that directory is empty for the change that added it). The GitHub octokit/webhook code lives in `apps/server` and calls shared mutators, never raw ZQL. |
...
| **Attachments** | Provider-neutral `StorageProvider` seam + two implementations (local filesystem volume by default; optional S3-compatible via `aws4fetch`) | MinIO-as-requirement is how Plane/Huly bloat their compose. `STORAGE_*` env flips to any S3-compatible endpoint (`S3_ENDPOINT` reaches R2/B2/Garage/an existing MinIO) for cloud/scale, and local is the *complete* default rather than a fallback. Same framework-plus-implementations shape as `Mailer`, and the same **no vendor SDK** judgement: `aws4fetch` is ~4 KB with zero dependencies and signs SigV4 over platform `fetch`, where `@aws-sdk/client-s3` is tens of megabytes for four verbs. The seam has exactly `put`/`get`/`delete`/`health` and **no `getUrl()` — there are no signed, presigned or shareable URLs of any kind, and no setting that turns them on.** An `<img src>` lives in a document that syncs, so any URL stored in one is a bearer capability replicated to every teammate's IndexedDB that also breaks the day it expires; the node carries an opaque id and the app proxies every byte, which is what keeps the permission check identical across both providers instead of provider-dependent and tested on only one. Enforced by a CI grep, not by intent. Content type is sniffed from the bytes against a five-format raster allowlist, so an SVG downloads rather than rendering in-origin. |
| **Search** | Postgres FTS, `tsvector` inside a GIN **expression** index on a server-only `search_document` sidecar; text configuration `simple` by default via `SEARCH_TEXT_CONFIG` | No Elasticsearch, ever (Huly's sprawl). **No `CREATE EXTENSION` of any kind** — `pg_trgm` was in the original row and is refused: it ships in the `postgres:18` image but needs a privilege some managed-Postgres self-hosters cannot grant, which trades the deployment promise for typo tolerance nobody asked for. So the server pass is exact, not fuzzy. The default is `simple`, not `english`: stemming would quietly optimise for English teams, and `simple` also sits closer to the client pass's literal semantics — a six-tier ladder of exact-key, prefix and substring matches ending in a strict word-prefix `abbreviation` tier (`cs` reaches "Change status"), never a fuzzy subsequence. The vector lives only in the index expression, so the table carries plain `text` and nothing exotic enters the logical-replication path toward zero-cache's replica. The index is maintained by a job on the existing pg-boss (never inside the write transaction — constraint #9 outranks search freshness), and the expression's configuration is a SQL *literal*, so the job rebuilds that one index when `SEARCH_TEXT_CONFIG` changes rather than silently degrading to a sequential scan. Bonus, and the first half of the feature: recent data is already client-side via Zero — the instant pass costs no network at all. |
...
| **Images** | sharp | Attachment thumbnails in the app container, generated **once at upload** (a read path that decodes is a read path where twenty concurrent thumbnails are a CPU DoS on a 2-vCPU VPS). **The first native module in the runtime image** — roughly +10 MB, no install script so no `allowBuilds` entry, and the prebuilt platform binaries carry across the Dockerfile's stages because build and runtime are both `node:24-slim` (same libc, same arch). The one new constraint on the release pipeline: a **cross-architecture** build must run on the target arch or configure pnpm's `supportedArchitectures`, and an Alpine base would need the musl variants — one more reason the base stays `node:24-slim`. |
...
| TipTap | **3.28.0 — pinned exactly, all ten catalog entries together** | `@tiptap/pm`, `@tiptap/react`, `@tiptap/starter-kit`, `@tiptap/extension-mention`, `@tiptap/suggestion`, `@tiptap/markdown`, `@tiptap/extension-image`, `@tiptap/extension-table`, `@tiptap/extension-code-block`, `@tiptap/extension-code-block-lowlight` — one exact version, never a caret range. The mention and suggestion packages carry **exact** peer ranges on `@tiptap/core`/`@tiptap/pm`, so a split resolution duplicates `prosemirror-model` and throws `RangeError: Adding different instances of a keyed plugin` **at runtime** — invisible to `typecheck` and to `build`. v3 extensions only; never mix v2/v3 packages. `@tiptap/core` is a *peer*, not a catalog entry, and is therefore **not resolvable from `packages/ui`** under pnpm's strict layout: import `Editor` and the core types from `@tiptap/react`, which re-exports them. `@floating-ui/dom` ^1.8.0 is a direct dependency because `@tiptap/suggestion` peer-requires it. **`@tiptap/markdown` is first-party (MIT, ~26 KB gzip, one runtime dep: `marked@^17`) and its output is CORRECTED, not used as shipped** — its serialiser HTML-entity-encodes all non-code text and escapes no block-leading characters, so `a < b & c` leaves as `a &lt; b &amp; c` and a paragraph reading `# not a heading` re-parses as a heading. `packages/ui/src/lib/markdown.ts` replaces one private method (`encodeTextForMarkdown`) to fix both — they are one defect, since `>` survives today only *because* of the entity encoding. **Do not "clean up" that replacement**; it asserts the method's existence at construction so a version bump fails loudly. Three further corrections sit on **public** surface and are equally load-bearing: the parse **refuses raw HTML** (`marked`'s `html`/`tag` tokenizers, switched off on the manager's `instance`, with `<em>` the one exception the serialiser itself emits) because `parse()` otherwise runs anything tag-shaped — including ordinary prose like `a<b and c>d` — through `generateJSON` against the whole extension set and can mint a mention node from pasted text; the code block's `renderMarkdown` sizes its fence to its content; and block-leading escapes de-indent first. The module lives in `packages/ui`, never `packages/schema`, because `apps/server` imports schema and this would drag the whole TipTap graph into the server — enforced by `scripts/check-boundaries.mjs` rule 3. **`@tiptap/extension-code-block` is DECLARED, not inherited from StarterKit**: `-code-block-lowlight` peer-requires it at exactly 3.28.0 and imports it at runtime, and under pnpm's strict layout an undeclared peer resolves inside starter-kit's own `node_modules` — a second `prosemirror-model`, and a throw at runtime after typecheck, lint and `vite build` have all passed. Verified API notes: [`reference/frontend-build.md` §11](reference/frontend-build.md) (§11.6 for the serialiser) |
| lowlight / highlight.js | 3.3.x / **11.11.x** | Syntax highlighting for the editor's code block, reached only through `@tiptap/extension-code-block-lowlight`. `lowlight` is MIT; **`highlight.js` is BSD-3-Clause — AGPL-compatible, and the first non-MIT/Apache runtime dependency in the client bundle**, so it is named here rather than passing unremarked. Only the curated grammars in `reference/frontend-build.md` §11.7 are registered — never `lowlight.common`, which is ~37 and would dominate the bundle. **No `highlight.js` stylesheet is ever loaded**: the ~250 it ships hard-code hex, and the `hljs-*` classes map to `--code-*` tokens asserted at AA in `styles/contrast.test.ts` |
...
| aws4fetch | 1.0.x | MIT, **zero dependencies, ~4 KB**. SigV4 signing over platform `fetch` for the optional S3 storage provider — the `resend.ts` no-vendor-SDK judgement applied verbatim, against `@aws-sdk/client-s3`'s tens of megabytes for four verbs. Server-only; nothing imports it under the `local` default |
...
- One-command backup/restore (`yapm backup` → pg_dump + attachments tarball). **The command is still unwritten; its contract is not.** `attachments` settled it and shipped it as a documented manual procedure ([Backup and restore](apps/docs/src/content/docs/self-hosting/backup-restore.md)): under the local provider a `pg_dump` **plus** a tar of `STORAGE_LOCAL_DIR`; under `s3` a `pg_dump` only, because versioning/lifecycle/replication are bucket features and streaming a whole bucket through the app container to make a tarball would be slower and more expensive than the thing it replaces — with the `attachment` table as the **manifest** an operator verifies their own bucket backup against. The database goes *first* both ways — dumped before the files are tarred, restored before they are unpacked — because an upload writes its object before its row, so every row in a dump names bytes that were already on disk. The tar is then a superset of what the dump refers to, and the extra objects are the orphans the nightly sweep already collects. The `zero-replica` volume is disposable and deliberately not backed up.
