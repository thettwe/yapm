...
1. **Phase 1 — PM core, graph-ready.** Issues, projects, cycles, triage, keyboard-first UI, GitHub sync (branch/PR ↔ issue), and **opt-in per-team status transitions** driven by that link — off by default, with divergence — the `//` break in the reality track — as the behaviour whenever they are off or blocked. The schema models the full work graph from day one even before all edges are ingested.
2. **Phase 2 — Delivery truth.** PR/CI/deploy ingestion, DORA + review-health + CI-health views. *Automatic status transitions were scoped here; the pull-request half of them shipped in Phase 1 instead, on opt-in terms, so what remains is deploy- and CI-driven transitions. The issue↔deployment edge they were waiting on now exists — an exact merge-commit join over a deploy timestamp that is written once and never overwritten — so the remaining question is a product one about whether a deploy should move a status, not a missing edge. The **review-health and CI-health views have now shipped** as the team Delivery view — team-level only, computed on the client from already-synced rows — and the deploy half is on it: every deployment that reached production is drawn along the cycle in progress. What the page deliberately does *not* show is deployment frequency **as a rate** — deployments are drawn as they happened rather than normalised — and it says so on the page, alongside change failure rate and MTTR, which still wait on the Phase 3 incident entity. Of the four DORA keys the view carries lead time only partially (open→merge).*
