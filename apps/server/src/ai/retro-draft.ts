...
- At most three proposals per bucket: at most three wins, at most three losses, at most three improvements, at most three follow_ups. Fewer is correct when the cycle says less; silence is better than filler.
...
- A follow_up reports the OUTCOME of one of the previous retro's agreed actions. Such a proposal MUST reference that action's id via refs with kind "retro_action"; one that does not is discarded.
- Report on the previous retro's agreed actions when, and only when, they are given to you below. Such a proposal MUST reference the action's id via refs with kind "retro_action", and MUST state the outcome exactly as yapm computed it — shipped, canceled, still open, or never tracked. Never assert an outcome yapm did not compute, and never claim an action shipped unless yapm says shipped. Give it the category that fits what the proposal is DOING: reporting how a prior action turned out is a follow_up, whatever the outcome was; asking the team to try the same thing again is an improvement, which may also cite the prior action it repeats.
