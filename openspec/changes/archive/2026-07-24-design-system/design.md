...
### Components phase (core component set + showcase; no schema/sync/provider work)

- **Scope drawn at the token boundary.** This phase built the strictly-tokenized components (§6) and the showcase (§7.1, §7.3), plus the `cmdk` dependency (§1.1's cmdk half; the fonts were already in place from the Tokens phase). It did NOT build the theme provider, the `user_preference` entity, the no-flash bootstrap, or `deriveAccent` — those remain the Accent (§3.1) and Sync (§4–5) phases. Consequently §7.2 (accent picker + app-shell theme switcher *wired to the provider*) is left for the Sync phase; this phase ships the showcase's own local preset/mode controls (which §7.3 requires) instead. The scaffold components (§2.5) needed no edits, as the Tokens phase already noted.
- **Command palette = `cmdk` surface inside a Base UI `Dialog`.** cmdk owns the filtered list, roving selection, and `data-selected` active row (styled to `--accent-soft`/`--accent-strong`); Base UI `Dialog` owns the overlay, Escape, focus trap, and focus return. This composes the two libraries rather than pulling cmdk's Radix-based `Command.Dialog`, keeping the overlay stack on Base UI. Verified in a headless render: ⌘K opens, typing filters (`assign` → 1 item), Escape closes.
- **Multi-color SVG glyphs reference tokens via CSS `var()` attributes.** Status glyphs use a single `currentColor` from a `text-status-*` utility; priority bars and the "done" check need two colours in one SVG, so unfilled bars use `fill="var(--text-3)"` and the done check uses `stroke="var(--bg)"`. These are token *references*, not hardcoded hex, preserving the tokens-only rule while allowing per-mark colour. Priority bars draw from `--text-2/3` and Urgent from `--status-urgent` (never the brand accent), matching Editorial's DIRECTION.
- **Issue-row labels take a `tone` enum mapped to status/accent token classes.** The DIRECTION label-dot hexes are not reproducible under tokens-only, so the primitive exposes `tone ∈ {neutral, accent, in-progress, in-review, done, urgent}` → a `text-*` token utility; `issue-core` can widen this to a real label-colour model later. Row height/density read `min-h-[var(--density-row)]` (token reference), so the row is 44/36/42px across Warm/Focused/Editorial with no source change.
- **Focusable row primitive.** The issue-row is a `tabIndex=0` `<div>` with an accent focus-visible ring + left rail and no assigned interactive role (consumer's job); the single `biome-ignore lint/a11y/noNoninteractiveTabindex` documents that intent.
- **Dev-only `/showcase` via mode-conditional `routeFileIgnorePattern`.** `apps/web/vite.config.ts` became a `({ command }) => …` config that ignores the `showcase` route file when `command === 'build'`, so the TanStack Router plugin regenerates the route tree without it and `dist` contains no `/showcase` (verified by grepping the prod build). Dev (`serve`) and `tsr generate` (typecheck/test) keep it. `routeTree.gen.ts` is git-ignored, so the divergent dev-vs-prod trees never cause churn. `vitest.config.ts` now calls the config function with a `serve` env to merge it.
- **Showcase self-scopes theming to a wrapper.** The route sets `data-theme` + `.dark` on its own wrapper `<div>` (not `document.documentElement`), relying on the Tokens phase's per-element `:root,[data-theme]` re-resolution, so it themes correctly without the (not-yet-built) provider touching the document root.

### Customize phase (accent customization + per-user persistence; §3.1, §4, §5, §7.2)

- **Accent Zod stays permissive; the parseable-color check throws a typed `MutationError` in the mutator body.** `defineMutator`'s validator wraps `.fn`, so a `z.refine(isParseableColor)` on the args would reject with an opaque Standard-Schema parse error before the body runs (and before the codebase's typed `MutationErrorCode` path). To match the established `createTeam`-invalid-key pattern and give a testable, code-carrying rejection on both client and server, `setPreferenceArgs.accent` is a plain `z.string().nullable()` and `preference.set` validates with `isParseableColor` in the body, throwing `MutationErrorCode.invalidColor`. Added that code to `errors.ts`.
- **`isParseableColor` lives in `packages/schema` (regex: hex/rgb()/oklch()); the sRGB + contrast math lives in `packages/ui` (`lib/color.ts`).** The schema layer only stores and validates the accent string and must stay UI-free (constraint 3), so it cannot import the ui color parser. The two are intentionally separate: schema validates *parseability*; ui computes *shades + AA-safe on-accent*. `deriveAccent` parses only hex/rgb to sRGB for the on-accent contrast pick and returns `color-mix()` strings for hover/active/soft/line (matching the preset CSS), so an oklch accent still validates and themes (its on-accent falls back to the higher-contrast of near-white/near-black via worst-case luminance).
- **on-accent = the higher-contrast of two fixed candidates (`#ffffff`, `#111111`).** Proven in `deriveAccent`/`onAccentFor`: picking max contrast coincides with the spec's WCAG-AA rule in every case (both pass → higher; neither passes → higher; exactly one passes → that one is the higher), so the user can never produce unreadable on-accent text. Unit-tested across dark/light/mid-tone accents.
- **Mode is device-local and NOT synced; `{theme, accent}` is synced.** The cache blob (`yapm:pref`) carries `{theme, mode, accent, + resolved shade strings}`; the inline `index.html` script applies it synchronously (verified in headless Chromium: seeded editorial+dark+accent paints before the bundle; empty cache → warm + `prefers-color-scheme`). The provider treats the synced `preferences.mine` row as source of truth for theme/accent, reconciles into device-local mode, rewrites the cache, and exposes optimistic setters that also call `preference.set` (UUIDv7 minted at the provider call site, not in the mutator).
- **`ThemeControls` (provider-wired) placed in the app-shell header; the showcase keeps a *local* accent picker.** The app-shell popover (preset `Select` + Light/Dark toggle button + hex `Input`/native color input + reset) drives the real synced provider and is fully keyboard-operable. The `/showcase` route intentionally keeps its own local preset/mode state (its purpose is previewing all six combos at once, which a single persisted preference cannot do), so its added accent picker applies `deriveAccent` to the route's wrapper `style` locally rather than mutating the document root — satisfying §7.2's "and the showcase" without fighting the persisted preference.
- **e2e (`apps/web/e2e/theme.spec.ts`) added.** No-flash (seeded cache + empty cache) and the prod-build-has-no-`/showcase` guard are verified here; the keyboard-persist test (sign in → keyboard preset/mode/accent → reload asserts persistence) and the existing auth/sync e2e need the full three-container + zero-cache stack and run in CI (not executed in this apply environment), same posture as the pre-existing `sync.spec.ts`.

### Close phase (final verification + review screenshots)

- **Full gate suite run green on a clean-volume three-container stack.** `pnpm turbo typecheck lint build test` = 14/14 tasks, 131 unit tests pass (schema 66, ui 27, web 6, server 32); the 2 schema-drift files that `skipIf` without a DB were then run explicitly against live Postgres (`DATABASE_URL=…@localhost:5440`) → 3/3 pass including `user_preference`. `node scripts/check-boundaries.mjs` clean. Playwright e2e against the live stack (postgres + zero-cache in docker on 5440/4848, server on 3210, vite on 5174) = **14/14** (auth 7, sync 4, theme 3 — no-flash, default-warm, keyboard-persist-across-reload), confirming no workspace-auth regression. `docker/docker-compose.yml` has exactly three services (`postgres`, `yapm`, `zero-cache`); the prod `apps/web/dist` contains no `/showcase` route.
- **Tasks 2.5 and 8.2 closed as done.** 2.5 was already satisfied without editing the six scaffold components — re-pointing `--primary`/`--ring`→`--accent`, `--accent-foreground`→`--on-accent`, `--muted`→`--bg-hover` through the contract aliases re-themes them (documented in the Tokens/Components phases); its checkbox was flipped to reflect that reality. 8.2's Playwright checks (no-flash, keyboard theme+accent persistence, auth/sync no-regression, prod-has-no-`/showcase`) are all green here.
- **Review screenshots saved to `design-explorations/built/`.** Six issue-list-mockup PNGs from `/showcase` — `showcase-{warm,focused,editorial}-{light,dark}.png` — plus `showcase-custom-accent.png` (Warm light with a user `#33bb77` accent, which recolors the selected-row tint/rail, the accent label dot, and derived shades from that single base). The three presets are visibly distinct (warm terracotta / focused iris + tighter 36px rows / editorial vermilion + taller 42px rows) and correct in both modes.
