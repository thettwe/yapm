...
- [x] 4.1 Add `octokit` + `@octokit/auth-app` + `@octokit/webhooks` (catalog) to `apps/server`; build the `App` (auth-app) client factory, enabled only when the App triplet is present (installation tokens minted on demand, never persisted)
- [x] 4.2 Webhook Hono route: raw-body capture, `X-Hub-Signature-256` HMAC verify (with fallback-secret rotation), enqueue + `202`; disabled-connector returns 404
