...
| `PROCESS.md` | **Updated** — §2's "mechanical checks" clause described two legs of `env-example.test.ts`; there are now four plus the `--env-file` check, which reads `README.md`, `SECURITY.md` and every page under `apps/docs/src/content/docs/`. |
