...
- [x] 1.1 Add `RETRO_PROPOSAL_CATEGORY_CHECK` to `packages/schema/src/zero/context.ts` as a plain
...
- [x] 1.2 Add `'follow_up'` to `RETRO_PROPOSAL_CATEGORIES` in
...
- [x] 1.3 Write `packages/schema/src/migrations/0022_retro_followup_category.ts`: drop
...
- [x] 1.4 Apply the migration against the live dev stack (`POSTGRES_HOST_PORT=5447
...
- [x] 2.1 Delete `retroProposalBucket`, `RETRO_PROPOSAL_BUCKETS`, `RetroProposalBucket` and
...
- [x] 2.2 Add `dropUnbackedFollowUps(content)` to `ai-draft.ts`: drop any proposal whose
...
- [x] 2.3 `packages/schema/src/zero/retro/ratify.ts`: `RatifiableProposal` carries an optional
...
- [x] 2.4 Confirm no `retroProposalBucket`, `RETRO_PROPOSAL_BUCKETS` or `BucketableProposal`
...
- [x] 3.1 `apps/server/src/ai/retro-draft.ts` — update `RETRO_DRAFT_SYSTEM_PROMPT` per design §D6:
...
- [x] 3.2 Verify the structured-output schema needs no separate edit: `retroDraftProposalSchema`
...
- [x] 3.3 `apps/web/e2e/db.ts` — widen the seeded proposal's `category` union to the four values so
      an e2e fixture can seed a follow-up.

## 4. The surface reads the stored value

- [x] 4.1 `apps/web/src/retro/ai-labels.ts` — both maps become `Record<RetroProposalCategory,
...
- [x] 4.2 `apps/web/src/retro/retro-ai-panel.tsx` — `groupByBucket`, the chip label, `BUCKET_UNIT`,
...
- [x] 4.3 Leave `actionable` in the panel and `focusedAi.category === 'improvement'` in
...
- [x] 4.4 Check every remaining `RetroProposalCategory` consumer compiles against four values:
...
- [x] 5.1 `ai-draft.test.ts` — **the falsifiable check**: `sanitizeRetroDraft` given a proposal with
...
- [x] 5.2 `ai-draft.test.ts` — four `follow_up` proposals in one draft are capped to three, and
...
- [x] 5.3 `ai-draft.test.ts` — an `improvement` citing a real prior action is stored as an
...
- [x] 5.4 `ratify.test.ts` — `contestedFirst` orders contested first, then the four categories in
      canonical order, then rank; a `{id, verdict}` row still ties on both legs.
- [x] 5.5 `packages/schema/src/db/schema-drift.test.ts` — assert the live
...
- [x] 5.6 A unit test pinning `RETRO_PROPOSAL_CATEGORY_CHECK` to name exactly the members of
      `RETRO_PROPOSAL_CATEGORIES`, so a fifth category added without a migration fails a test rather
      than an insert (design §D2).
- [x] 5.7 `retro-ai-panel` test — a proposal row storing `improvement` and carrying a
...
- [x] 6.1 `apps/docs/src/content/docs/features/retro-ai-draft.md` — the follow-up section: a
...
- [x] 6.2 `ROADMAP.md` — a row for this change, and a superseding note on row 22's "**`follow_up` is
...
- [x] 6.3 Check `README.md`, `TECHSTACK.md`, `.env.example` and `reference/` for staleness — expected
