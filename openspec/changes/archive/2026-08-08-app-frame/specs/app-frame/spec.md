...
The three-band chrome every authenticated page in yapm sits inside: a deck of destinations
that never adapts to the page, a masthead the page owns and adapts, and a statusline that
states the team's day. It is the product's navigation contract — six destinations, one
attention number, no route unreachable, no fact fabricated where none is in context.
