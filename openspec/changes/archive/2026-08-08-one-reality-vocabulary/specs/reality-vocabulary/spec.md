...
The one drawn language every yapm surface uses to say what is true about a change — delivery
reality as a track of stations with a `//` break where the board and git disagree, status as
cycle position, priority as weight, plus the three shared patterns that language depends on: the
peek, the `how ·` footnote, and the provenance mark.
...
The status glyph SHALL draw **cycle position** — one loop filled as far as the work has run:
backlog a dashed ring, todo an open ring, in-progress a half arc, in-review a three-quarter arc, and
done a filled disc. The priority mark SHALL draw **weight** as ticks, with unfilled ticks quieted
rather than absent, and with one tick standing alone denoting urgent.
