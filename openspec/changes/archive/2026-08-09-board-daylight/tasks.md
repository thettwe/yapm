...
- [x] 5.9 Re-run any e2e failure once before investigating: the known multi-context flake (`projects.spec.ts:188`, `:246`, `pm-digest.spec.ts:306`, signature `browserContext.close: Protocol error`) is tracked separately and is not this change's to fix. Any OTHER failure is *(the re-run-once policy is visible in CI history for PR #44: runs 31290241661 and 31292309700 both carry `run_attempt=2`. Every failure across runs 31287526824 / 31290241661 / 31292309700 is either the tracked `browserContext.close: Protocol error (Target.disposeBrowserContext)` signature or the sibling retro change's `retro-ai.spec.ts:552` flake — no `board.spec.ts` failure in any run. Merged head 6da63e89 is green on run 31295789682)*
...
- [x] 7.1 `pnpm turbo lint typecheck test build` *(green on CI run 31295789682, head 6da63e89 = merged tree, 2m51s)*
- [x] 7.2 The compose smoke test *(pass on CI run 31295789682, 4m47s)*
- [x] 7.3 The full Playwright suite *(pass on CI run 31295789682, 22m48s — full suite, no grep)*
- [x] 7.4 **Render task.** Bring the built page up at 1440×900 over a seeded team, screenshot it, and compare it against `board.png` / `board-full.png`. Record **every** deliberate difference in `design.md` — the `↓ n more` fold (D4) is a known one; find the rest *(rendered 2026-08-10 over seeded teams, built bundle; six fluid columns at 1440 with no horizontal scroll confirmed; sheet at scratchpad/render4 (49 shots: degenerates in warm/light, plus 3 presets x light/dark over board, cycles, home, projects, roadmap, retro room, and a focus capture per block))*
- [x] 7.5 **Render the degenerate states and look at them**: a board with exactly one card; a board where every column is empty; a column with 40 cards; a card whose title runs long; and a card with a phrase but no track. Each must read as composed, not as a hole *(all rendered and looked at 2026-08-10: the one card sits alone with five reserved dashed slots and true zero counts; the all-empty board is six reserved slots, no words; the 40-card column states 40 and scrolls with no fold; the ~240-char title wraps ~8 lines inside its clamped measure without breaking the column's rhythm; nothing needed fixing. The phrase-no-track card was not seeded — it needs a linked PR row and is the one sub-state still unrendered.)*
