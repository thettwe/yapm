...
- [x] 3.3 Check legibility at the dense-row size (`size-3.5`, 14px) in the showcase route across all six theme blocks. If the shared 1.6px stroke is illegible there, step the check's stroke up by one value and record the exact value and the reason in `design.md` §"Decisions made during implementation" (design D4) *(stroke held at the shared 1.6 with the ratio argument in §DI-6; the eyes-on step across six presets is still owed)* *(DONE: rendered the showcase glyph row at deviceScaleFactor 4 in all three presets, light and dark. The check reads clearly as a check on the filled disc; stroke left at the shared 1.6.)*
...
- [x] 5.6 Look at the delivery charts once after the retune — `--chart-1` aliases this token (design D6 risk). Record what you saw *(measured in design.md §DI-5 — ribbon, ribbon outline and added-block outline all re-measured and recorded; eyes-on confirmation is 8.4)* *(DONE: Delivery rendered over a seeded team with 57 historical pull requests — the amber reads as amber on the cycle-flow bars and the added-block outlines, and is not confusable with --status-done green or --status-urgent.)*
...
- [x] 8.1 `pnpm turbo lint typecheck test build` — report the actual output, never a claim *(fast gates run locally and green — typecheck 3/3, `biome ci` 667 files clean, `@yapm/ui` 19 files / 440 tests, `@yapm/web` 55 files / 589 tests, `check-boundaries.mjs` OK, `@yapm/docs build` 35 pages. `build` itself is left to CI on the push, which runs the whole gate)* *(green on CI run 31269155758)*
- [x] 8.2 The compose smoke test *(pass on CI run 31269155758, 4m37s)*
- [x] 8.3 The full Playwright e2e suite. `apps/web/e2e/projects.spec.ts` has known timeout flake independent of this change: re-run before diagnosing, and do not loosen it *(pass on CI run 31269155758, 21m7s)*
- [x] 8.4 Open the showcase route and step every one of the six theme blocks: the done glyph's check at BOTH the sizes the product draws it — `size-3.5` (14px, a dense row) and `size-[13px]` (the Delivery peek's chip and panel, the smallest any surface draws, and the size §DI-6's stroke argument has to answer for) — a quiet issue row, a populated row beside it, the retuned amber on the status glyph, the attention square and the delivery charts *(DONE: showcase stepped through all six theme blocks at 4x; the done check is legible, the retuned amber stays amber and stays distinct from green and from urgent in every block.)*
...
- [x] 8.6 Fill in `design.md` §"Decisions made during implementation" with every call made along the way, and check that every task box above is honestly ticked *(design.md carries §"Decisions made during implementation" DI-1..DI-6; the eyes-on boxes above are now ticked against a real render rather than an argument.)*
