...
- [x] 11.5 Verify in all three presets, light and dark, that no ink on either page is colour-only and that focus is visible *(6-block sheet produced 2026-08-10 with a focus capture per block; sampled editorial-dark closely: the three column marks each pair a disc with its word, the dot budget draws dots AND keeps the `3/3` mono reading, the phase machine names its bands, and the focus ring is clearly drawn on the dark ground. Full sheet at scratchpad/render4 (49 shots: degenerates in warm/light, plus 3 presets x light/dark over board, cycles, home, projects, roadmap, retro room, and a focus capture per block) for the maintainer's own pass. One truthful adaptation observed: an attributed room states `cards carry their author` where an anonymous one states the no-author-column guarantee — the register tells the truth either way)*
