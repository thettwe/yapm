...
- [x] 1.1 Add `aws4fetch: ^1.0.20` to the `catalog:` block in `pnpm-workspace.yaml`, alphabetically,
...
- [x] 1.2 Add the storage block to `apps/server/src/config/env.ts`: `STORAGE_PROVIDER`
...
- [x] 1.3 Add the all-or-nothing refinement on the `GITHUB_APP_VARS` precedent: `STORAGE_PROVIDER=s3`
...
- [x] 1.4 **Test (unit)** extend `apps/server/src/config/env.test.ts`: defaults with nothing set;
...
- [x] 2.1 Write `packages/schema/src/migrations/0017_attachments.ts` — **`0017`, not `0016`**, which
...
- [x] 2.2 Add `attachment` to the hand-written Kysely `DB` interface in
...
- [x] 2.3 Add the `attachment` table to the Zero schema in `packages/schema/src/zero/schema.ts` with
...
- [x] 2.4 Add `attachments.byIssue` to `packages/schema/src/zero/queries.ts`, wrapped in
...
- [x] 2.5 Create `packages/schema/src/db/attachment.ts` with the server-only accessors the REST path
...
- [x] 2.6 **Test (integration)** extend `packages/schema/src/db/schema-drift.test.ts` so the table
      and every column are asserted present in Postgres and in the Zero schema.
- [x] 2.7 **Verify on the live stack before building on it** (the `search` I1 / `auto-status` I1
...
- [x] 3.1 Create `apps/server/src/storage/provider.ts`: the `StorageProvider` interface with exactly
...
- [x] 3.2 Create `apps/server/src/storage/local.ts`: `createLocalStorageProvider({ dir })`.
...
- [x] 3.3 Create `apps/server/src/storage/s3.ts`: `createS3StorageProvider({ bucket, region,
...
- [x] 3.4 Create `apps/server/src/storage/index.ts` with `createStorageProvider(env, logger)` on the
...
- [x] 3.5 **Test (unit)** `apps/server/src/storage/local.test.ts` and `s3.test.ts`: round-trip
...
- [x] 4.1 Create `apps/server/src/storage/sniff.ts`: a fixed magic-number table for PNG, JPEG, GIF,
...
- [x] 4.2 Create `apps/server/src/storage/thumbnail.ts`: `sharp` at `limitInputPixels: 268402689`
...
- [x] 4.3 **Test (unit)** `sniff.test.ts`: each allowlisted format from real magic bytes; an SVG, an
...
- [x] 4.4 **Test (unit)** `thumbnail.test.ts`: a generated PNG produces a WebP within the bound; a
      non-image buffer returns `null`; an image smaller than the bound is not enlarged; EXIF is
      absent from the output.

## 5. The routes

- [x] 5.1 Create `apps/server/src/storage/routes.ts` exporting `createFileRoutes(options)` and
...
- [x] 5.2 `GET /:id` and `GET /:id/thumb`: parse → session → **single scoped statement** →
...
- [x] 5.3 `POST /` under `hono/body-limit` at `ATTACHMENT_MAX_BYTES`: session → team membership with
...
- [x] 5.4 `PATCH /:id` (attach once, from null, same team) and `DELETE /:id` (object, thumbnail, row;
      idempotent). Both refuse with the same `REFUSAL` for anything the caller may not see.
- [x] 5.5 Mount in `apps/server/src/app.ts` as an optional `files?: Hono` beside `search`, and build
      it in `apps/server/src/index.ts` from the provider, the db and the auth service.
- [x] 5.6 Add a `storage` readiness check calling `provider.health()` to the `readinessChecks` array
      in `index.ts`, following the existing checks' shape.

## 6. Deployment surface

- [x] 6.1 `docker/Dockerfile`: create `/var/lib/yapm/files` in the runtime stage and `chown` it to
      `yapm:yapm` **before** `USER yapm`, so a bind-mounted host path is not silently unwritable by
      uid 1001.
- [x] 6.2 `docker/docker-compose.yml` and `docker/docker-compose.dev.yml`: a `files` named volume
...
- [x] 6.3 Update `.env.example` with the full storage block, commented: local is the default and is
...
- [x] 7.1 Create `apps/server/src/jobs/attachments.ts`: `ATTACHMENT_GC_QUEUE` and
      ...
      never rejects.
- [x] 7.2 Register it as a fourth independent block in `apps/server/src/jobs/scheduler.ts`:
...
- [x] 7.3 **Test (unit)** `jobs/attachments.test.ts` against a fake provider and a fake db: an
...
- [x] 7.4 **Test (unit)** extend `scheduler`'s existing topology test — with the injected `boss` —
...
- [x] 8.1 Create `apps/server/src/storage/no-capability.test.ts`, structured like
...
- [x] 8.2 Add a self-check to that file, on the isolation test's precedent: assert the derived member
      list is non-empty and contains `put`, so a parse that silently returns nothing cannot make the
      guard vacuously pass.

## 9. Tests

- [x] 9.1 **Test (integration, live Postgres)** `apps/server/src/storage/routes.pg.test.ts` — **the
