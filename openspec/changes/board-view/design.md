...
- **Docs app stood up now, not deferred.** Task 8.1 originally deferred docs because `apps/docs` was a pre-Starlight stub. The docs gate (PROCESS.md §2) requires the page to ship with the behavior, so this change scaffolds the minimum Astro Starlight app (`astro.config.mjs`, `src/content.config.ts`, catalog `astro`/`@astrojs/starlight`/`sharp`) and adds a user-facing **Board** page under a Features section plus a home index. Scope was kept to what makes `pnpm --filter @yapm/docs build` pass — the full docs site (evaluator/self-hoster/contributor trees) remains a future docs-site change.
- **Docs app has no `typecheck` script; build only.** Per `reference/frontend-build.md`, TS7 has no Compiler API, so Volar-based tooling — including `astro check` — cannot run; a docs typecheck would force a `typescript@6` alias that is out of scope for a single feature page. The docs `build` (`astro build`, Vite/esbuild, no compiler API) is the enforced gate; turbo simply skips the absent `typecheck` task for `@yapm/docs`. Astro's generated `.astro/` dir is gitignored and excluded from Biome (alongside `dist`/`.turbo`) so its emitted `any`/`{}` types and unformatted output do not fail lint.
