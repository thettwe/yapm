...
- [ ] 1.1 Write `packages/schema/src/migrations/0023_deploy_history.ts` (highest on main is `0022`):
...
- [ ] 1.2 `packages/schema/src/db/types.ts` — `DeploymentTable` gains `sha: Nullable<string>` and
...
- [ ] 1.3 `packages/schema/src/zero/schema.ts` — `deployment` gains `sha: string().optional()` and
...
- [ ] 1.4 `packages/schema/src/db/schema-drift.test.ts` — add the three rows to the expected column
...
- [ ] 2.1 `packages/schema/src/zero/work-graph.ts` — `upsertDeployment` gains `readonly sha: string
...
- [ ] 2.2 `applyWorkGraphMutation`'s `upsertDeployment` branch implements design §D2's three rules:
...
- [ ] 2.3 The stale-event path stops returning blind: when `mutation.sourceUpdatedAt <
...
- [ ] 2.4 `upsertPullRequest` persists `mergeCommitSha` on both insert and update, alongside
...
- [ ] 3.1 `apps/server/src/connectors/github/payloads.ts` — `GithubPullRequest` gains
...
- [ ] 3.2 `apps/server/src/connectors/github/map.ts` — the `deployment_status` case maps
...
- [ ] 3.3 `apps/server/src/connectors/github/reconcile.ts` — `reconcileDeployments` emits
...
- [ ] 3.4 Confirm no other producer of `upsertDeployment` / `upsertPullRequest` exists (grep both
...
- [ ] 4.1 `packages/schema/src/zero/delivery.ts` — `DeliverySignal` gains `deployedAt: number |
...
- [ ] 4.2 `computeDeliverySignal` returns `deployedAt` = the **earliest** `deployedAt` among
...
- [ ] 4.3 `assembleLinkedEntities` gains an optional second parameter — the team's deployment rows
...
- [ ] 4.4 `packages/schema/src/zero/filter.ts` — `merged-not-deployed` becomes
...
- [ ] 5.1 `packages/ui/src/components/issue-row.tsx` — `RealityStripProps` gains `deployedAt: number
...
- [ ] 5.2 `apps/web/src/issues/delivery.ts` — `linkedEntitiesFor` takes the team's deployment rows
...
- [ ] 5.3 `apps/web/src/issues/issue-list.tsx` + `model.ts` — subscribe to
...
- [ ] 5.4 Do not touch `apps/web/src/routes/`, `apps/web/src/retro/seed-model.ts`,
...
- [ ] 5.5 `apps/web/src/issues/issue-detail.tsx` — its `DeploymentRow` and repo-filtered deploy list
      keep working unchanged; add nothing to it. Confirm by compile, and say so.

## 6. Tests

- [ ] 6.1 **The falsifiable check.** `packages/schema/src/zero/work-graph.test.ts`: apply
...
- [ ] 6.2 `work-graph.test.ts` — three successive deployments to one environment (three external
...
- [ ] 6.3 `work-graph.test.ts` — redelivery: apply the identical success mutation twice;
...
- [ ] 6.4 `work-graph.test.ts` — the stale-success path (§D2): a success at T1 arriving AFTER an
      ...
      `deployedAt`.
- [ ] 6.5 `work-graph.test.ts` — `sha` is written on insert, preserved when a later mutation omits
      it, and `mergeCommitSha` round-trips on `upsertPullRequest`.
- [ ] 6.6 `apps/server/src/connectors/github/map.test.ts` — a `deployment_status` fixture that
...
- [ ] 6.7 `apps/server/src/connectors/github/reconcile.test.ts` — the mocked `listDeployments`
...
- [ ] 6.8 **Both directions in one test.** `packages/schema/src/zero/delivery.test.ts` (or the
...
- [ ] 6.9 `packages/schema/src/zero/filter.test.ts` — **deliberately replace**
      ...
      was wrong.
- [ ] 6.10 `packages/ui` / `apps/web` component test — a row with a `deployedAt` renders the deploy
