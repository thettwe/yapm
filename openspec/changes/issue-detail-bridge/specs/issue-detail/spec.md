...
The surface SHALL retain, in both its sheet and its full-page form, every capability it carries: the rich-text description with autosave, mentions and image attachments; the Files section; the comment thread with its composer; and the properties block covering status, priority, assignee, cycle, labels and the follow control.
