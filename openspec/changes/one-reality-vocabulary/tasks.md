...
- [ ] 1.1 Read `design-explorations/overhaul-2026-08/northstar/ia.html` (§"Two patterns, drawn
      once", §"Provenance") and `northstar/NORTHSTAR.md`.
- [ ] 1.2 Read `issues.html` (`<defs>` for the arcs/ticks; `.track`/`.tn`/`.tseg`/`.tbreak`/`.t-age`
...
- [ ] 1.3 Read `reference/zero.md` (Zero 1.x names, not 0.x) and the Tailwind 4.3 reference note.
- [ ] 1.4 Read `packages/ui/src/components/{issue-row,status-glyph,priority-mark,board-card}.tsx`,
...
- [ ] 2.1 Collapse `RealityStripProps` (`packages/ui`) and `TeamHomeStrip`
...
- [ ] 2.2 Add the generalized track model to `packages/ui`: exported `TrackNodeKind`,
...
- [ ] 2.3 Implement `RealityTrack` with composable `orientation` (`horizontal` | `vertical`),
...
- [ ] 2.4 Accessibility: horizontal track is `role="img"` with the composed truthful label
...
- [ ] 2.5 Verify: `pnpm turbo lint typecheck` green; the app still builds and runs.

## 3. Arcs and ticks reconciled to the mock geometry

- [ ] 3.1 Rewrite `packages/ui/src/components/status-glyph.tsx` to the `issues.html` `<defs>`
...
- [ ] 3.2 Rewrite `packages/ui/src/components/priority-mark.tsx` to the mock's tick geometry —
...
- [ ] 3.3 Verify: every existing consumer of both components compiles and renders unchanged in API
      terms; `pnpm turbo lint typecheck` green.

## 4. Promote the drawn primitives into one shared module

- [ ] 4.1 Move `DayBand`, `ScopeBand`, `TickBar`, `TriageDots` and `CadenceChart` from
...
- [ ] 4.2 Refactor `apps/web/src/home/team-home.tsx` onto the shared module — both `RealityTrack`
      call sites (`:432` broken form → the divergence-derived shape; `:772` the YOURS row) and every
      band primitive.
- [ ] 4.3 **Delete** `apps/web/src/home/drawn.tsx` and `apps/web/src/home/cadence-chart.tsx` — no
      re-export shim, no duplicate copy anywhere in the repo.
- [ ] 4.4 Verify: `rg` for a second implementation of any drawn primitive returns nothing; team home
      renders identically to before (same bands, same evidence).

## 5. Retire the icon strip and migrate every consumer

- [ ] 5.1 Delete `RealityStrip`, `RealityStripPlaceholder`, `DivergenceFlag`, `PR_GLYPH`, `CI_GLYPH`
...
- [ ] 5.2 `apps/web/src/issues/issue-list.tsx` — swap the primitive at `:546`, drop the
      `divergenceFlag` prop at `:548`, keep the row's present layout. Pass the divergence kind into
      the shape so the break is drawn.
- [ ] 5.3 `apps/web/src/issues/issue-detail.tsx` — `DeliveryDetail` draws the horizontal track;
...
- [ ] 5.4 `apps/web/src/issues/delivery.ts` — `DeliveryView.strip` takes the shared type.
- [ ] 5.5 `packages/ui/src/components/board-card.tsx` — the placeholder becomes the track's empty
      shape at the card's measure.
- [ ] 5.6 `apps/web/src/routes/showcase.tsx` — render the new vocabulary; the divergence-triangle row
      becomes a broken-track row.
- [ ] 5.7 Verify: `pnpm turbo lint typecheck build` green; `rg "RealityStrip|DivergenceFlag|CI_GLYPH|CI_HEALTH_GLYPH"`
...
- [ ] 7.1 Rewrite `packages/ui/src/components/issue-row.test.tsx` onto the track: the empty track
      reserves the same width as a populated one; a diverged shape draws the break; the accessible
      label states the facts drawn.
- [ ] 7.2 Unit test `buildRealityShape` — each of the three divergence kinds puts the break on a
      different segment; the four facts map to the documented node kinds; no fifth axis is drawn.
- [ ] 7.3 Unit test the shared-type assignability guard (design §D3): the UI's mirrored `PrState` /
...
- [ ] 7.8 Update the e2e selectors that name the retired slot — `apps/web/e2e/connectors.spec.ts`
