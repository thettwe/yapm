...
- [x] 1.1 Add `AI_DISCLOSURE_EVENTS` (`policy_changed | generated | published | unpublished`) and the
...
- [x] 1.2 Write `packages/schema/src/migrations/0021_pm_digest.ts` creating `pm_digest`: `id` uuid pk;
...
- [x] 1.3 In the same migration create `ai_disclosure_audit`: `id` uuid pk; `workspace_id` uuid not
...
- [x] 1.4 Register `0021_pm_digest` in `packages/schema/src/migrations/index.ts` (do NOT take `0020`
...
- [x] 2.1 Add the `pm_digest` table to `packages/schema/src/zero/schema.ts` with exactly these
      columns: `id`, `teamId`, `cycleId`, `status`, `content` (optional json), `provider`, `model`,
      `generatedAt`, `publishedAt`, `audienceSizeAtPublish`, `createdAt`, `updatedAt`. **Exactly ONE
      relationship, `team`** (see design I3 — `teamScoped` correlates through it and a
      relationship-free table cannot be team-scoped at all; the enforceable rule is that no query may
      call `.related(...)`, asserted over every PM query's AST). Carry a header comment stating D1 and D8: the row relates to
...
- [x] 2.2 Do NOT add `input_token`, `output_token`, `estimated_cost_usd` or `published_by` to the
      sync schema, and do NOT add `ai_disclosure_audit` at all.
- [x] 2.3 Extend `packages/schema/src/db/schema-drift.test.ts`: the two new table shapes, four new
...
- [x] 3.1 Extend `aiConfigDataSchema` in `packages/schema/src/db/ai-config.ts` with the
...
- [x] 3.2 Add `packages/schema/src/db/pm-disclosure.ts` with `resolvePmAudienceTeamIds(db, userId):
...
- [x] 3.3 Add `recordDisclosureAudit(db, entry)` to the same module — the ONE writer of
...
- [x] 3.4 Add `setPmDisclosurePolicy(db, ctx, options)` — admin-gated through `upsertAiConfig` (which
...
- [x] 3.5 Extend `getWorkspaceAiSpendUsd` in `packages/schema/src/db/cycle-digest.ts` with a fourth
...
- [x] 4.1 Add `readonly pmAudienceTeamIds?: readonly string[]` to `AuthContext` in
...
- [x] 4.2 Add `pmAudienceScoped` to `packages/schema/src/zero/queries.ts`, immediately below
      ...
      ever merged.
- [x] 4.3 Add the `pmDigests` query group (audience axis, `pmAudienceScoped`): `byCycle({cycleId})`
...
- [x] 4.4 Add the `pmDigestReview` query group (producing team, `teamScoped`): `byCycle({cycleId})`
...
- [x] 4.5 Do not modify `teamScoped` or any existing query. Confirm with `git diff` that the only
      change to `queries.ts` is additive.

## 5. Credential plumbing

- [x] 5.1 Extend `createSessionContextResolver` in `apps/server/src/zero/context.ts` with a
...
- [x] 5.2 Wire the resolver in `apps/server/src/app.ts` / `index.ts` to `resolvePmAudienceTeamIds`.
- [x] 5.3 Return the resolved audience from the sync-credential endpoint in
      `apps/server/src/auth-routes.ts` beside `role`.
- [x] 5.4 Carry it through `apps/web/src/zero/session.ts` (parse defensively, exactly as `asRole`
...
- [x] 6.1 Add `pmDigest.publish({id})` and `pmDigest.unpublish({id})` to
...
- [x] 6.2 Add server overrides in `packages/schema/src/zero/server-mutators.ts` that, through the
...
- [x] 6.3 Register both in the client `mutators` map and confirm `pm_digest` content/status/provider/
      model/token/cost fields have NO client mutator anywhere.

## 7. Generation

- [x] 7.1 Add `packages/schema/src/zero/pm-digest.ts`: re-export `digestContentSchema` as the
...
- [x] 7.2 Add `packages/schema/src/zero/pm-digest-writes.ts` with the server-only
...
- [x] 7.3 Add `apps/server/src/ai/pm-digest.ts`: `PM_DIGEST_SYSTEM_PROMPT` (PM altitude — outcomes and
...
- [x] 7.4 Write one `generated` audit row on every terminal status, attributed to the system
      principal.
- [x] 7.5 Extend the `CYCLE_DIGEST_QUEUE` worker in `apps/server/src/jobs/scheduler.ts` to run the PM
...
- [x] 7.6 Add `AI_PM_DIGEST` (default `false`) to `apps/server/src/config/env.ts` with the D-I1
...
- [x] 10.1 **Unit** — `pmAudienceScoped`: denies for a non-member, denies for an empty or absent
...
- [x] 10.2 **Unit** — `resolvePmAudienceTeamIds` over each of the four switches, including the legacy
      config blob parsing to all-off.
- [x] 10.3 **Unit** — `buildPmDigestInput` contains no identity-shaped key at any depth; the gateway
      is called with no tools; `buildPmEvidenceLabels` renders `ENG-142 · PR #331` and yields nothing
      for an unknown id.
- [x] 10.4 **Unit** — the publish/unpublish mutator gates: a viewer is rejected before any existence
...
- [x] 10.5 **Integration (pg)** — the falsifiable check, in one file: a workspace member with
...
- [x] 10.6 **Integration (pg)** — one `ai_disclosure_audit` row per generation, per publish, per
