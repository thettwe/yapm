...
- [ ] 1.1 Add `AI_DISCLOSURE_RETENTION_DAYS` (`z.coerce.number().int().min(1).max(3650).default(365)`)
...
- [ ] 1.2 Add `AI_PM_DIGEST_READY_EMAIL` (`'true' | 'false'`, default `'false'`) beside
...
- [ ] 1.3 Add the three variables to the env-var description map used by the fail-fast reporter, and
...
- [ ] 2.1 `deleteDisclosureAuditOlderThan(db, before)` in `packages/schema/src/db/pm-disclosure.ts`,
      deleting from `ai_disclosure_audit` only and returning the row count — the shape of
      `deleteNotificationsOlderThan`.
- [ ] 2.2 `AI_DISCLOSURE_RETENTION_QUEUE` and `runDisclosureRetention` in a new
...
- [ ] 2.3 Register it in `startScheduler` as an independent block on the **shared** boss —
...
- [ ] 2.4 Wire it in `apps/server/src/index.ts` unconditionally (it is not gated on AI, on the
      disclosure switches or on a mailer — design D3).

## 3. The publish-time ready notice

- [ ] 3.1 Add `'pm_digest_published'` to `NOTIFICATION_KINDS` and `'pm_digest'` to
...
- [ ] 3.2 Extend `notificationCopy` with an actor-free branch: title "A cycle digest was shared with
      you", summary from the yapm-computed team and cycle names. No actor label, no content.
- [ ] 3.3 In the `pmDigest.publish` server override, after the release is stamped, fan out one
...
- [ ] 3.4 Add the `pm_digest` case to the inbox's `open()` navigation (`/digests`) so the row is not
      a dead end, keeping the existing `subjectType` switch exhaustive.

## 4. Ready-email delivery

- [ ] 4.1 `packages/email/src/pm-digest-ready.tsx`: `renderPmDigestReady({ publicUrl, teamName,
...
- [ ] 4.2 Narrow `pendingNotificationEmails` with `subject_type = 'issue'`, commented as the clause
      that makes the two selections disjoint. It removes no row today.
- [ ] 4.3 `pendingPmDigestReadyEmails` beside it in `db/notification.ts`: `subject_type =
...
- [ ] 4.4 `AI_PM_DIGEST_READY_QUEUE` and `runPmDigestReadyEmailSweep` in `jobs/disclosure.ts`: per
...
- [ ] 4.5 Register it on the shared boss only when a mailer, a public URL and
      `AI_PM_DIGEST_READY_EMAIL=true` are all present, reusing `NOTIFICATION_EMAIL_CRON`; wire it in
      `index.ts`.

## 5. The admin audit view

- [ ] 5.1 `disclosureAuditLogForWorkspace(db, workspaceId)` in `db/pm-disclosure.ts`, modelled on
...
- [ ] 5.2 `GET /api/v1/ai/disclosures` in `ai/admin-routes.ts` behind the existing `requireAdmin`,
      beside `/verdicts`, with the comment saying it refuses before any read and that there is no
      companion write.
- [ ] 5.3 Client types and `fetchAiDisclosureLog()` in `apps/web/src/settings/ai.ts`, mirroring the
      verdict-log pair.
- [ ] 5.4 A read-only `DisclosureAuditSection` on the AI settings page: per-team totals, the recent
...
- [ ] 5.5 Section copy states what the view is and is not, in the product's own words: what was
...
- [ ] 6.1 `packages/schema/src/db/pm-disclosure-retention.pg.test.ts`: a record older than the window
...
- [ ] 6.2 `packages/email/src/pm-digest-ready.test.tsx`: render the notice for a digest whose content
...
- [ ] 6.3 Sweep unit tests: with `mailer: null` the ready sweep is a total no-op and throws nothing;
...
- [ ] 6.4 Scheduler topology test: the disclosure retention queue is registered on the shared boss
...
- [ ] 6.5 Admin-route tests: a member and a viewer are refused before any read, and the refusal in a
      ...
      list.
- [ ] 6.6 `apps/web/src/settings/ai-disclosure-log.test.tsx`: the section renders totals and events,
      renders nothing when the log is empty, and exposes no per-person aggregate.
- [ ] 6.7 `apps/server/src/config/env-example.test.ts`: the `.env.example` key set equals the Zod
      schema key set modulo the two declared, commented exception lists (design D10).
- [ ] 6.8 Notification disjointness: `pendingNotificationEmails` selects no `pm_digest`-subject row
      ...
      mailed once.
- [ ] 6.9 Extend `apps/web/e2e/pm-digest.spec.ts` with the inbox half: after a publication, the named
      reader's inbox holds the notice, it names no publisher, and its rendered text contains no
      substring of the digest content.

## 7. Documentation

- [ ] 7.1 New `apps/docs/src/content/docs/self-hosting/ai-disclosure.md` — the disclosure model end
...
- [ ] 7.2 Update `features/pm-digest.md` for the governance half, now using "auditable" and
      "retention-bounded" accurately and saying what each actually means.
- [ ] 7.3 Update `self-hosting/ai-setup.md` and `self-hosting/email.md` with the three new variables
      and the second sweep.
- [ ] 7.4 Update `README.md`: move governance out of "What's next" into shipped, and sweep the
      paragraphs change 20 wrote around the two forbidden words.
- [ ] 7.5 Update `ROADMAP.md` row 23 to built, and check the "Where v1 actually stands" narrative
      tells the truth now that the AI-features family is complete.
- [ ] 7.6 Update `VISION.md` where it describes the AI posture, and `.env.example`.
- [ ] 7.7 Update `PROCESS.md` §2 so its mechanical-check claim points at the check that now exists.

## 8. Verification

- [ ] 8.1 `pnpm turbo lint typecheck test build` clean.
- [ ] 8.2 `node scripts/check-boundaries.mjs` clean (packages never import apps).
- [ ] 8.3 The pg suites run against a live Postgres, and the compose smoke test passes.
