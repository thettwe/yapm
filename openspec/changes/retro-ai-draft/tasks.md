...
- [ ] 1.1 Add `AI_ARTIFACT_STATUSES` (`['pending','ready','failed','ai_off']`) and
...
- [ ] 1.2 Create `packages/schema/src/zero/ai-content.ts`: the normalized artifact view
...
- [ ] 1.3 Rewrite `dropUncitedItems`, `dropItemsNamingMembers` and `contentNamesMember` in
...
- [ ] 1.4 **Test (unit, no DB)** `packages/schema/src/zero/ai-content.test.ts`: the walkers over a
      ...
      group.
- [ ] 1.5 Make `getWorkspaceAiSpendUsd` (`packages/schema/src/db/cycle-digest.ts:9`) sum the `ready`
...
- [ ] 1.6 Add rule 4 to `scripts/check-boundaries.mjs`: fail on a second definition of
...
- [ ] 1.7 Run `pnpm turbo lint typecheck test` and `node scripts/check-boundaries.mjs`. Everything
      green with zero behaviour change is the gate for group 2.

## 2. Migration `0018_retro_ai` and the schema surface

- [ ] 2.1 Write `packages/schema/src/migrations/0018_retro_ai.ts`: `team.ai_retro_draft_since
      ...
      `migrations/index.ts`.
- [ ] 2.2 Add both tables and the team column to the hand-written Kysely `DB` interface in
      `packages/schema/src/db/types.ts`.
- [ ] 2.3 Add both tables to `packages/schema/src/zero/schema.ts` with their `team` relationships
...
- [ ] 2.4 Add `retroAiDrafts.byRetro({retroId})` and `retroAiProposals.byRetro({retroId})` to
...
- [ ] 2.5 **Test (integration)** extend `packages/schema/src/db/schema-drift.test.ts`: both tables
...
- [ ] 2.6 **Verify on the live stack before building on it** (the `attachments` I1 / `search` I1
...
- [ ] 3.1 Create `packages/schema/src/zero/retro/ai-draft.ts`: `RETRO_PROPOSAL_CATEGORIES`,
...
- [ ] 3.2 In the same file, `sanitizeRetroDraft(content, knownIds, roster)`: the three validators in
      the order design §D6 fixes — cite-or-omit, then name-drop, then cap. Pure and synchronous.
- [ ] 3.3 Add the server-only write helpers to `packages/schema/src/zero/retro/ai-draft-writes.ts`
...
- [ ] 3.4 Export everything new from `packages/schema/src/index.ts` (and the `/server` entry for the
      write helpers, if that is where `retro/server-writes.ts` is reachable from). Run
      `node scripts/check-boundaries.mjs`.
- [ ] 3.5 **Test (unit, no DB)** `packages/schema/src/zero/retro/ai-draft.test.ts`: a hallucinated
...
- [ ] 4.1 Create `packages/schema/src/db/retro-facts.ts` with
...
- [ ] 4.2 Assemble and return `{ teamId, cycleId, cycleName, seed, issues, evidenceIds }`, where
      ...
      union as `citableIds`.
- [ ] 4.3 **Do not restructure `packages/schema/src/zero/cycle-facts.ts`.** `feat/pm-digest-areas` is
      extending `buildCycleFacts` concurrently; consume whatever shape `main` has after the rebase.
- [ ] 4.4 **Test (integration)** `packages/schema/src/db/retro-facts.pg.test.ts`: seed a team with a
...
- [ ] 4.5 **Test (integration)** in the same file, the **table/column allowlist**: wrap the Kysely
...
- [ ] 5.1 Add the shared mutator `team.setAiRetroDraft` to `packages/schema/src/zero/mutators.ts`:
...
- [ ] 5.2 Classify it in `MUTATOR_TOOL_KINDS` (`write`) and add its args entry in
      `packages/schema/src/zero/ai-tools.ts` — that registry is exhaustive by construction and its
      test fails otherwise.
- [ ] 5.3 In `packages/schema/src/zero/server-mutators.ts`, extend the existing
...
- [ ] 5.4 **Test (unit)** extend `packages/schema/src/zero/mutators.test.ts`: `setAiRetroDraft`
...
- [ ] 5.5 **Test (integration)** extend `packages/schema/src/zero/mutators.retro.pg.test.ts`: with
...
- [ ] 6.1 Create `apps/server/src/ai/retro-draft.ts`: `RETRO_DRAFT_SYSTEM_PROMPT` (the
...
- [ ] 6.2 In the same file, `runRetroAiDraft(deps, input)` — `runCycleDigest`'s state machine,
...
- [ ] 6.3 Create `apps/server/src/jobs/retro-draft.ts`: `runRetroAiDraftTail({db, dbProvider,
...
- [ ] 6.4 Register the block in `apps/server/src/jobs/scheduler.ts`, copying `SEARCH_INDEX_QUEUE`
...
- [ ] 6.5 Add `AI_RETRO_DRAFT` (`'true'|'false'`, default `'true'`) to
...
- [ ] 6.6 Add rule 5 to `scripts/check-boundaries.mjs`: `apps/server/src/ai/retro-draft.ts` and
      `apps/server/src/jobs/retro-draft.ts` may not import `buildAgentTools` or `runAgent`. Fixture
      test alongside rule 4's.
- [ ] 6.7 **Test (unit)** `apps/server/src/ai/retro-draft.test.ts` with a mocked gateway: the
...
- [ ] 6.8 **Test (unit)** `apps/server/src/jobs/retro-draft.test.ts`: the claim statement is issued
...
- [ ] 7.1 **Test (integration)** `apps/server/src/ai/retro-draft.pg.test.ts` — design §D11, all six
...
- [ ] 7.2 Run it against the live compose stack on the ports above and paste the actual output into
      design.md §Decisions. A green claim without the output is not a green claim.
- [ ] 7.3 Confirm `packages/schema/src/zero/queries.anonymity.pg.test.ts` still passes with the two
