...
- [x] 1.1 Add `'retro_action'` to `RETRO_SEED_REF_KINDS` in
...
- [x] 1.2 Add the outcome vocabulary and the prior-retro fact shapes to
...
- [x] 1.3 Extend `RetroFacts` in `packages/schema/src/db/retro-facts.ts` with
...
- [x] 1.4 Implement the read in `retroFactsForCycle`, using the prior-cycle list it already computes
...
- [x] 1.5 Add every prior action id to `citableIds`, and add a prior-retro totals key to the citable
...
- [x] 2.1 Add `retroProposalBucket` to `packages/schema/src/zero/retro/ai-draft.ts`: pure, exported,
...
- [x] 2.2 Change `capRetroProposals` and `rankRetroProposals` to bucket by `retroProposalBucket`
      ...
      is still an integer.
- [x] 2.3 Point change 19's ordering comparator (`packages/schema/src/zero/retro/ratify.ts`) at
...
- [x] 2.4 Verify no second definition of "is this a follow-up" appears anywhere: `retroProposalBucket`
...
- [x] 3.1 Extend `RETRO_DRAFT_SYSTEM_PROMPT` in `apps/server/src/ai/retro-draft.ts` with the
...
- [x] 3.2 Extend `buildRetroDraftInput` with the prior-retro block: the cycle those actions were
...
- [x] 3.3 After `sanitizeRetroDraft` and before the write, overwrite the `label` of every surviving
...
- [x] 4.1 Render a fourth group in `apps/web/src/retro/retro-ai-panel.tsx`, grouped by
...
- [x] 4.2 Render a `retro_action` reference as a plain non-navigating chip carrying the baked label
      and an outcome marker. It is not a link: the prior retro's rows are not synced into this view
      and this change adds no query.
- [x] 4.3 Extend the category chip and change 19's flat contested-first list to use
      `retroProposalBucket`, so a follow-up is labelled a follow-up in both renderings.
- [x] 4.4 Every colour and font through a semantic token; correct and AA in Warm, Focused and
...
- [x] 5.1 Add a team-level verdict read to `packages/schema/src/db` (a new module beside
...
- [x] 5.2 Add one admin-gated `GET` to `apps/server/src/ai/admin-routes.ts` under the existing
...
- [x] 5.3 Add the client call to `apps/web/src/settings/ai.ts` and render the log as a section in
...
- [x] 5.4 No regenerate control, no per-team quality setting, no prompt editor. It is a read.

## 6. Tests

- [x] 6.1 **The falsifiable check.** In `packages/schema/src/db/retro-facts.pg.test.ts`: seed a prior
...
- [x] 6.2 Grow `ALLOWED_TABLES` in that file by exactly `retro` and `retro_action` and keep the
