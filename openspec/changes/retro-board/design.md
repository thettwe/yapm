...
Mutation-checked both ways. Dropping the membership `whereExists` from `teamScoped` fails both new tests and **no other test in the repository at this tier**; restoring the D-32 defect in the harness fails the signed-out test **alone**. Each therefore guards something that was previously unguarded rather than restating a check that already existed.
