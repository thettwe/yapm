...
- [ ] 1.1 Add `RETRO_REACTION_VALUES` (`agree | disagree`), `RETRO_PROPOSAL_VERDICTS`
...
- [ ] 1.2 Write `packages/schema/src/migrations/0020_retro_ratification.ts`: create
...
- [ ] 1.3 Extend the hand-written Kysely `DB` interface in `packages/schema/src/db/types.ts` with
      `RetroAiReactionTable` and the new columns on `RetroAiProposalTable` and `RetroActionTable`.
- [ ] 1.4 Add the `retro_ai_reaction` table to the Zero schema in
...
- [ ] 2.1 Add `'react'` to `RETRO_WRITE_OPS` with `ALLOWED_PHASES.react = ['group', 'vote']` in
      `packages/schema/src/zero/retro/phase.ts`. Change nothing else in that file.
- [ ] 2.2 Write `packages/schema/src/zero/retro/ratify.ts`: the pure `retroProposalVerdict(agree,
      disagree)` implementing design §D3, and a pure contested-first comparator over proposal rows.
      No I/O, no ZQL.
- [ ] 2.3 Add `setRetroAiReaction` and `clearRetroAiReaction` to
...
- [ ] 2.4 Add the optional `aiProposalId` to `createRetroActionArgs`, validated to name a proposal in
      the same retro, defaulting to null. Do not touch `convertRetroActionToIssue`.
- [ ] 2.5 Add `queries.retroAiReactions.mine({ retroId })` to `packages/schema/src/zero/queries.ts`
      ...
      `retroVotes.mine` (~:313).
- [ ] 2.6 Export the new types, constants, query and mutator names from `packages/schema/src/index.ts`.

## 3. Ratification at the phase advance

- [ ] 3.1 Write the server-only helpers beside change 18's `ai-draft-writes.ts`:
...
- [ ] 3.2 Wire both into the existing `retro.setPhase` override in
...
- [ ] 4.1 Add the reaction controls to `apps/web/src/retro/retro-ai-panel.tsx`: two `aria-pressed`
      ...
      stamp.
- [ ] 4.2 Add the verdict display from `discuss` onward — a tokenized badge plus the agree/disagree
...
- [ ] 4.3 Add the one-keystroke "add as action" control on an agreed improvement, calling
      `retroAction.create` with `aiProposalId` and **no assignee**, and thread it through
      `apps/web/src/retro/api.ts`.
- [ ] 4.4 Thread the retro phase into `RetroAiPanel` from `apps/web/src/retro/retro-view.tsx`, and
...
- [ ] 5.1 Unit: `ratify.test.ts` — the verdict function over the full table of (agree, disagree)
