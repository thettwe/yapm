...
## Purpose
The Board lens: a team's issues laid out as six fixed status columns, where a card says exactly what
a list row says in a different shape and a move is drawn rather than animated. It is a lens on the
same synced rows the list reads — same filter bar, same delivery seam, same counts — so the two can
never disagree, and the whole surface is operable without a pointer. Archived from change
board-view and extended by board-daylight (PR #44).
